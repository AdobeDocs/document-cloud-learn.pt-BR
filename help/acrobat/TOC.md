---
user-guide-title: Tutorials Acrobat
solution: Acrobat
breadcrumb-title: Tutorials Acrobat
feature-set: Document Cloud
role: User
type: Tutorial
auto-video-transcripts: true
source-git-commit: 3a947947f9eaac8e369a2236b5795fb5a74fb01b
workflow-type: tm+mt
source-wordcount: '634'
ht-degree: 4%

---


# Tutorials Acrobat {#acrobat-learning}

+ [Visão geral](overview.md)
+ Introdução {#get-started}
   + [Visão geral](getting-started/getting-started-overview.md)
   + Fundamentos {#basics}
      + [Noções básicas do Workspace](getting-started/workspace-basics.md)
      + [Nova experiência do Acrobat](getting-started/new-experience.md)
      + [Trabalhar com  [!DNL Microsoft 365]](https://experienceleague.adobe.com/docs/document-cloud-learn/acrobat-learning/integrations/integrate-overview.html?lang=pt-BR#microsoft)
      + [Trabalhe em qualquer lugar com o Acrobat Web](getting-started/acrobatweb.md)
      + [Produtividade em qualquer lugar](getting-started/productivity.md)
      + [De onde vêm os PDF?](getting-started/where-do-pdfs-come-from.md)
   + Trabalhe de modo mais inteligente com IA {#ai}
      + [Descubra insights de PDF com o AI Assistant](getting-started/ai-assistant.md)
      + [Aumente a eficiência da equipe com o PDF Spaces](getting-started/pdf-spaces-legal.md)
      + [O que é o Acrobat Studio?](getting-started/acrobat-studio.md)
   + Criar e organizar {#create}
      + [Criar um PDF](getting-started/create-pdf.md)
      + [Combine arquivos em um único PDF](getting-started/combine-to-pdf.md)
      + [Organizar páginas](getting-started/organize.md)
      + [Projete uma nova página](getting-started/add-custom-page.md)
      + [Papel para PDF](getting-started/scan-and-ocr.md)
   + Editar e aprimorar {#edit}
      + [Editar texto em um PDF](getting-started/edit-pdf.md)
      + [Editar gráficos em um PDF](getting-started/edit-graphics.md)
      + [Estilizar esta PDF](getting-started/stylize-this-pdf.md)
      + [Ajuste automático de layout](getting-started/auto-adjust-layout.md)
      + [Converter PDF em diferentes formatos de arquivo](getting-started/export-pdf.md)
   + Colaborar e revisar {#collaborate}
      + [Colaborar em tempo real](getting-started/collaborate.md)
      + [Comentar em um PDF](getting-started/comment-on-pdf-files.md)
   + Forms, assinaturas e proteção {#tools}
      + [Criar formulários preenchíveis](getting-started/create-fillable-forms.md)
      + [PDF forms Fill &amp; Sign](getting-started/fill-and-sign.md)
      + [Protect um arquivo PDF com uma senha](getting-started/password-protect.md)
      + [Obter assinaturas](getting-started/signatures.md)
      + [Rastrear seus documentos](getting-started/track.md)
+ Série de acessibilidade {#accessibility-tutorials}
   + [Visão geral](accessibility-series/accessibility-overview.md)
   + [O quê e por que dos arquivos de PDF acessíveis](accessibility-series/what-why-accessible-pdf.md)
   + [Configurar seu espaço de trabalho](accessibility-series/set-up-workspace.md)
   + [Criar um PDF acessível do Word](accessibility-series/create-accessible-from-word.md)
   + [Crie um PDF acessível no Acrobat](accessibility-series/accessible-acrobat.md)
   + [Noções básicas de marcação](accessibility-series/basics-tagging.md)
   + [Documentos digitalizados](accessibility-series/scanned-documents.md)
   + [Criar formulários acessíveis](accessibility-series/create-accessible-forms.md)
   + [Listas complexas](accessibility-series/complex-lists.md)
   + [Tabelas complexas](accessibility-series/complex-tables.md)
   + [Ação guiada Tornar acessível](accessibility-series/make-accessible-action.md)
+ Tarefas avançadas {#advanced-tasks}
   + [Visão geral](advanced-tasks/advanced-tasks-overview.md)
   + Preparar e aprimorar {#prepare}
      + [Compactar e otimizar um PDF](advanced-tasks/reduce.md)
      + [Otimizar documentos digitalizados](advanced-tasks/optimizescan.md)
      + [Optimize PDF para SEO](advanced-tasks/optimizeseo.md)
      + [Adição de marcadores e hiperlinks](advanced-tasks/bookmarks.md)
      + [Detectar diferenças entre dois PDFs](advanced-tasks/compare.md)
      + [Aprimorar seu PDF](advanced-tasks/enhance.md)
   + Criar e gerenciar formulários {#forms}
      + [Trabalhar com campos de formulário](advanced-tasks/workforms.md)
      + [Campos de formulário avançados](advanced-tasks/advancedforms.md)
      + [Trabalhar com dados de formulário](advanced-tasks/formdata.md)
   + Proteger {#protect}
      + [Remover informações confidenciais](advanced-tasks/redact.md)
   + Automatizar e dimensionar workflows {#automate}
      + [Ações guiadas](advanced-tasks/action.md)
      + [Comandos e ferramentas personalizados](advanced-tasks/custom.md)
+ Alimentado por IA {#ai-powered}
   + [Visão geral](ai-powered/ai-overview.md)
   + [Descubra os insights do PDF com o Assistente de IA](https://experienceleague.adobe.com/pt-br/docs/document-cloud-learn/acrobat-learning/get-started/ai/ai-assistant)
   + [Aumente a eficiência da equipe com o PDF Spaces](https://experienceleague.adobe.com/pt-br/docs/document-cloud-learn/acrobat-learning/get-started/ai/pdf-spaces-legal)
   + [O que é o Acrobat Studio?](https://experienceleague.adobe.com/pt-br/docs/document-cloud-learn/acrobat-learning/get-started/ai/acrobat-studio)
   + [Editar elementos gráficos em uma PDF](https://experienceleague.adobe.com/pt-br/docs/document-cloud-learn/acrobat-learning/get-started/edit/edit-graphics)
   + [Estilizar esta PDF](https://experienceleague.adobe.com/pt-br/docs/document-cloud-learn/acrobat-learning/get-started/edit/stylize-this-pdf)
+ Casos de uso {#use-cases}
   + [Visão geral](use-cases/use-cases-overview.md)
   + Governo {#gov}
      + [Visão geral](use-cases/gov/gov-overview.md)
      + Cinco dicas para trabalhar em qualquer lugar com o Acrobat {#tips}
         + [5 dicas para trabalhar em qualquer lugar com o Acrobat](use-cases/gov/5-tips-for-working-anywhere-with-acrobat-dc-for-government.md)
         + [Dica #1 - Obtenha suas ferramentas](use-cases/gov/get-your-tools.md)
         + [Dica #2 - Colaborar com documentos](use-cases/gov/collaborate-on-documents.md)
         + [Dica #3 - Proteger documentos digitais](use-cases/gov/protect-digital-documents.md)
         + [Dica #4 - Trabalhar com formulários e assinaturas](use-cases/gov/work-with-forms-and-signatures.md)
         + [Dica #5 - Digitalizar e editar em dispositivos móveis](use-cases/gov/scan-and-edit-on-mobile.md)
+ Acrobat de 60 segundos {#60-second}
   + [Visão geral](60-second/60-second-overview.md)
   + [Editar o PDF com o Acrobat na Web](60-second/edit.md)
   + [Reconhecer texto em um arquivo PDF digitalizado](60-second/textrecognition.md)
   + [Combinar arquivos em uma PDF](60-second/combine-to-one-pdf.md)
   + [Organizar páginas num piscar de olhos](60-second/organize.md)
   + [Editar uma foto no PDF](60-second/editphoto.md)
   + [Editar um gráfico no seu PDF](60-second/editgraphic.md)
   + [Converter uma PDF em Word](60-second/convert-pdf-word.md)
   + [Converter um PDF em Excel](60-second/convert-pdf-excel.md)
   + [Converter um PDF em PowerPoint](60-second/convert-pdf-powerpoint.md)
   + [Export PDF para Word do seu telefone](60-second/exportwordphone.md)
   + [Criar uma PDF a partir do Microsoft Word](60-second/word-to-pdf.md)
   + [Criar arquivos PDF com o Acrobat](60-second/create-from-acrobat.md)
   + [Converter Word para PDF, incluindo campos de formulário](60-second/wordform.md)
   + [Criar uma PDF de fotos em um instante](60-second/photo.md)
   + [Converter um arquivo PPT em PDF no seu telefone](60-second/phone.md)
   + [Criar arquivos PDF mais eficientes num piscar de olhos](60-second/optimize.md)
   + [Assinar eletronicamente um documento em papel](60-second/sign.md)
   + [Proteger seus arquivos PDF com uma senha](60-second/protect.md)
   + [Redação: o caminho certo](60-second/redaction.md)
   + [Compartilhar para comentar](60-second/share-comment.md)
   + [Compartilhar e comentar em arquivos do PDF no Teams](60-second/share-comment-teams.md)
   + [Disputa entre comentários do PDF e Resumir](60-second/summarize-comments.md)
   + [Carregar comentários do PDF no InDesign](60-second/indesign.md)
   + [Deixe o Acrobat ajudá-lo a criar PDF acessíveis](60-second/accessible.md)
   + [Conformidade de uma PDF a um formato padrão](60-second/conform.md)
   + [Detectar as diferenças com o PDF Compare](60-second/compare.md)
   + [Pesquisar vários arquivos do PDF de uma só vez](60-second/search.md)
+ Construtores de habilidades {#skill-builders}
   + [Webinars](skill-builder/skill-builder-webinars.md)
+ Integrações {#integrations}
   + [Visão geral](integrate/integrate-overview.md)
   + [Criar PDF a partir de [!DNL Microsoft Word]](integrate/createfromword.md)
   + [Criar PDFs em [!DNL Office] para a Web](integrate/createofficeweb.md)
   + [Colaboração do PDF em  [!DNL Microsoft Teams]](integrate/acrobatandteams.md)
   + [Trabalhar com seus  [!DNL SharePoint]  arquivos](integrate/acrobatandsp.md)
   + [Converter mensagens de email e anexos em PDF em  [!DNL Outlook]](integrate/outlook.md)
   + [Criar conteúdo do PDF ao navegar com  [!DNL Microsoft Edge]](integrate/edge.md)
   + [Proteger PDFs usando  [!DNL Microsoft Purview Information] rótulos de sensibilidade](integrate/microsoftsensitivitylabels.md)
   + [Adobe Acrobat para  [!DNL Google Drive]](integrate/acrobatandgoogle.md)
   + [Trabalhar com arquivos de  [!DNL Dropbox]](integrate/acrobat-dropbox.md)
+ Desenvolver {#develop}
   + [Visão geral](develop/develop-overview.md)
+ Implantar {#deploy}
   + [Visão geral](deploy/deploy-overview.md)
   + [Importantes atualizações de produtos do Acrobat DC para clientes da ETLA](deploy/signentitlementchanges.md)
+ Dispositivos móveis {#mobile}
   + [Visão geral](mobile/mobile-overview.md)
   + [Produtividade em movimento](https://experienceleague.adobe.com/pt-br/docs/document-cloud-learn/acrobat-learning/get-started/basics/productivity)
   + [Digitalizar qualquer coisa no PDF](mobile/scan-mobile-app.md)
